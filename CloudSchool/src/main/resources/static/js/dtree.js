...
// ������ Tree object
function dTree(objName, objbool) { // void ���췽��������������objName:
									// �����ơ�objbool����ѡ��ѡ true��ѡ false��ѡ
									// Ĭ�ϵ�ѡ
	this.config = {
		...
		inOrder : false
	// ������ڵ��������ӽڵ�֮ǰ������������true�и��õ�Ч�ʣ�Ĭ��false
	}
	this.icon = {
		root : '/static/img/base.gif', // Node 根节点。
		folder : '/static/img/folder.gif', // Node 根节点。
		folderOpen : '/static/img/folderopen.gif', // 打开的文件夹,默认'img/folderOpen.gif'
		node : '/static/img/page.gif', // 打开的文件夹,默认'img/folderOpen.gif'
		empty : '/static/img/empty.gif', // 打开的文件夹,默认'img/folderOpen.gif'
		line : '/static/img/line.gif', // 竖线,默认'img/line.gif'
		join : '/static/img/join.gif', // 丁线,默认'img/join.gif'
		joinBottom : '/static/img/joinbottom.gif', // 直角线,默认'img/joinbottom.gif'
		plus : '/static/img/plus.gif', // 加号丁线,默认'img/plus.gif'
		plusBottom : '/static/img/plusbottom.gif', // 加号直角线,默认'img/plusbottom.gif'
		minus : '/static/img/minus.gif', // 减号丁线,默认'img/minus.gif'
		minusBottom : '/static/img/minusbottom.gif', // 减号直角线,默认'img/minusbottom.gif'
		nlPlus : '/static/img/nolines_plus.gif', // 减号直角线,默认'img/minusbottom.gif'
		nlMinus : '/static/img/nolines_minus.gif' // 无线减号,默认'img/nolines_minus.gif'
	};
	...
	if (node.pid == this.root.id) {
		...
		if (node.cchecked) {
			...
			if (this.RadioRocheckbox) {
				...
				checkboxSyntax = "<label class='form-check-label'><input class='form-check-input' type='checkbox' desc='"
						+ node.cshow
						+ "' name='"
						+ node.cname
						+ "' id='"
				...
				checkboxSyntax = "<label class='form-check-label'><input class='form-check-input' type='radio' desc='"
						+ node.cshow
						+ "' name='"
						+ node.cname
						+ "' id='"
			...
			checkboxSyntax += ">"
					+ "<span class='form-check-sign dtree_node' node_id='"
					+ node.id + "'>" + node.cshow + "</span></label>";
			/** ��װcheckbox���� */
		}
		str += checkboxSyntax;
	}
...
// ����Ƿ����κ�һ���ڵ�ĺ�����������һ���ֵܽ��� -- Checks if a node has any children
...
dTree.prototype.doRemove = function(obj, event) {
	...
	if (x > left + width - 9) {
		...
		if (this.RadioRocheckbox) {
			var $chk = $('.dtree_Tree a input[type="checkbox"][value="'
					+ userid + '"]');
			if ($chk.length > 0) {
	...
	for (i = 0; i < obj.length; i++) {
		if (obj[i].checked) {
			...
			if ($("#ulSelected #user_" + obj[i].value + "").length > 0) {
				...
				var strU = '<li id="user_'
						+ obj[i].value
						+ '"  uid="'
						+ obj[i].value
						+ '" name="'
						+ obj[i].getAttribute("cshow")
						+ '" mytype="2"> <div class="selectedUser" onmouseover="showRemove(this)" onmouseout="hideRemove(this)" onmousemove="setRemove(this,event)" onclick="javascript: '
						+ this.obj
						+ '.doRemove(this,event);" style="cursor: pointer;">'
						+ obj[i].getAttribute("cshow") + ' </div></li>';
...
dTree.prototype.isHaveBNode = function(id, pid) {
	...
	for (var n = 0; n < this.aNodes.length; n++) {
		...
		if (this.aNodes[n].pid != -1 && this.aNodes[n].id != id
			...
			if (eval("document.all." + this.aNodes[n].cname + "_"
					+ this.aNodes[n].id + ".checked"))
				isChecked = true;
		}
	}
	return isChecked;
...
dTree.prototype.checkPNodeRecursion = function(pid, ischecked) {
	for (var n = 0; n < this.aNodes.length; n++) {
		if (this.aNodes[n].pid != -1 && this.aNodes[n].id == pid) {
			eval("document.all." + this.aNodes[n].cname + "_"
					+ this.aNodes[n].id + ".checked = " + ischecked);
...
// [Cookie]���򿪽ڵ��id��Ϊ�ַ������� -- [Cookie] Returns ids of open nodes as a
// string
...
// [Cookie]���һ���ڵ�id�Ƿ���Cookie��-- [Cookie] Checks if a node id is in a
// cookie
...
// ���Push��pop�����������ʵ�ֵ�-- If Push and pop is not implemented by the
// browser
