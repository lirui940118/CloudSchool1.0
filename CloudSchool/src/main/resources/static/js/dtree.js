...
dTree.prototype.node = function(node, nodeId) {
	...
	if(node.url) {
		str += '<a id="s' + this.obj + nodeId + '" class="demo ' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" href="' + node.url + '"';
	...
	if(node.pid == this.root.id) {
		...
		if(node.cchecked) {
			...
			if(this.RadioRocheckbox) {
				...
				checkboxSyntax = "<label class='form-check-label'><input class='form-check-input' type='checkbox' desc='" + node.cshow + "' name='" + node.cname + "' id='" + node.cname + "_" + node.id + "' cshow='" + node.cshow + "'  value='" + node.cvalue + "' onClick='javascript: " + this.obj + ".checkNode(" + node.id + "," + node.pid + "," + node._hc + ",this.checked);' ";
			} else {
				checkboxSyntax = "<label class='form-check-label'><input class='form-check-input' type='radio' desc='" + node.cshow + "' name='" + node.cname + "' id='" + node.cname + "_" + node.id + "' cshow='" + node.cshow + "' value='" + node.cvalue + "' onClick='javascript: " + this.obj + ".checkNode(" + node.id + "," + node.pid + "," + node._hc + ",this.checked);' ";
			}
			if(node.cchecked)
			...
			checkboxSyntax += ">" + "<span class='form-check-sign dtree_node' node_id='" + node.id + "'>" + node.cshow + "</span></label>";
			/**��װcheckbox����*/
		}
		str += checkboxSyntax;
	}
...
dTree.prototype.checkNode = function(id, pid, _hc, checked) {
	...
	for(i = 0; i < obj.length; i++) {
		if(obj[i].checked) {
			...
			if($("#ulSelected #user_" + obj[i].value + "").length > 0) {} else {
